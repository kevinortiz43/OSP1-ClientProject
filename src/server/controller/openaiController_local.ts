...
async function fastTextSearch(query: string): Promise<{
  ...
  if (!keywords) {
    return { results: [], source: 'search-db', shouldUseAI: needsAI };
  }
  ...
  if (cachedResults.length > 0) {
    ...
    return { results: cachedResults, source: 'search-cache', shouldUseAI: needsAI };
  }
  ...
  return { results: result.rows, source: 'search-db', shouldUseAI: needsAI };
}
...
// ── Main Function (replaces Express middleware) ───────────────────────────────
